...
export default function TemplateModal({ isOpen, onClose, onSelectTemplateForChat }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4">
      <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden border border-slate-200">
        {/* Header Modal */}
        <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between bg-slate-50 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Persetujuan & Pengajuan Template WhatsApp Cloud API</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50/60">
          {activeTab === 'list' ? (
            <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                ...
                {filteredTemplates.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className={
                        'p-3.5 rounded-xl border transition cursor-pointer text-left ' +
                        (isSelected
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        <span className="font-mono text-xs font-bold text-slate-800 truncate block">{item.name}</span>
              ...
              {/* Kolom Pratinjau WhatsApp (Kanan) */}
              <div className="md:col-span-6">
                <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm sticky top-0">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-3">
                    ...
                    <span className="text-[11px] font-mono text-slate-400">ID: {previewTemplate?.id}</span>
                  </div>

                  <div className="bg-[#efeae2] p-4 rounded-xl shadow-inner min-h-[190px] flex flex-col justify-center">
            ...
            <form
              ...
              className="max-w-2xl mx-auto bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4"
            >
              <div>
                <h3 className="text-sm font-bold text-slate-800">Formulir Pengajuan Template Meta</h3>
                <p className="text-xs text-slate-500">
                  Template yang diajukan akan ditinjau otomatis oleh Meta AI dalam 1-15 menit.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Kategori Template Meta:</label>
                  <select
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-bold text-slate-700">Isi Pesan (Body):</label>
                  <span className="text-[11px] text-slate-400">
                    Gunakan <code className="text-emerald-700 bg-emerald-50 px-1 rounded">{'{{1}}'}</code>,{' '}
                    <code className="text-emerald-700 bg-emerald-50 px-1 rounded">{'{{2}}'}</code> untuk teks variabel
                  </span>
                </div>
                <textarea
                  rows={5}
